...
indicador_dam <- readRDS('01 Modelo de area//MEX/2020/Data/base_modelo.Rds')
...
# anti_join(statelevel_censo,statelevel_satelite) %>% select( dam2 )
# anti_join(statelevel_satelite, statelevel_censo) %>% select( dam2, dam )
statelevel_predictors_df <- inner_join(statelevel_satelite, statelevel_censo)
...
names_cov <-c( "dam2",
  "luces_nocturnas",
              ...
              "cubrimiento_urbano",
              "modificacion_humana",
              "tasa_desocupacion","discapacidad1","rezago_escolar") 
X_pred <- anti_join(statelevel_predictors_df[,names_cov],
...
saveRDS(fit_mcmc2,
        "01 Modelo de area//MEX/2020/Data/fit_multinomial_con_covariable2.Rds")

fit_mcmc2 <- readRDS("01 Modelo de area//MEX/2020/Data/fit_multinomial_con_covariable.Rds")

## Valores estimados para beta

summary(fit_mcmc2)$summary
...
dim(posterior_theta)
rowsrandom <- sample(dim(posterior_theta)[1], 100)
